...
pub struct LinkOutcome {
    /// Slid runtime address to dispatch the guest at — for `LC_MAIN`
    /// binaries, this is the `main()` function. The caller is responsible
    /// for setting `x0..x3` to `(argc, argv, envp, apple)` before
    /// dispatching, since with no dyld in between, `main` is entered
    /// directly with its argument-passing convention.
    pub entry: u64,
}

/// Link the guest in-process: apply chained fixups or classic dyld_info
/// bind/rebase opcodes, set up TLV thunks, and return the entry address.
/// The `image` must already be mapped (segments at `vmaddr + slide`); we
/// read the load commands directly from that mapping.
pub fn link(image: &LoadedMachO) -> Result<LinkOutcome, Error> {
    let cmds = LoadCommands::parse(image)?;

    let entry = cmds
        .entry_offset
        .map(|off| image.mh_addr.wrapping_add(off))
        .ok_or_else(|| Error::Link("guest has no LC_MAIN entry point".into()))?;
    ...
    Ok(LinkOutcome { entry })
}

fn load_dependent_dylibs(cmds: &LoadCommands) {
    let trace = std::env::var("CHIMERA_DYLD_TRACE").is_ok();
    for path in &cmds.dylibs {
        ...
        if trace {
            eprintln!("chimera: dlopen {} -> {:p}", path, handle);
        }
        // A NULL handle just means we couldn't find/load this dylib — the
        // bind path will still fail with a clear error if the guest
        // actually needs a symbol from it.
    }
}
...
struct LoadCommands {
    ...
    #[allow(dead_code)]
    dylibs: Vec<String>,
    /// `LC_MAIN.entryoff` (file offset of `main` from the start of the
    /// Mach-O), if present.
    ...
    fn parse(image: &LoadedMachO) -> Result<Self, Error> {
        ...
        for _ in 0..header.ncmds {
            ...
            match lc.cmd {
                ...
                LC_MAIN => {
                    let ep: EntryPointCommand = unsafe { ptr::read_unaligned(cursor as *const _) };
                    entry_offset = Some(ep.entryoff);
                }
...
fn apply_chained_fixups(
    ...
    if header.imports_format != DYLD_CHAINED_IMPORT {
        return Err(Error::Unsupported(format!(
            "chained-fixups imports format {} (only DYLD_CHAINED_IMPORT)",
...
fn parse_imports(
    ...
    let mut names = Vec::with_capacity(header.imports_count as usize);
    for i in 0..header.imports_count {
        let raw: u32 =
            unsafe { ptr::read_unaligned((imports_base + (i * 4) as u64) as *const u32) };
        // DYLD_CHAINED_IMPORT bitfield:
        //   lib_ordinal :  8
        //   weak_import :  1
        //   name_offset : 23
        let name_off = raw >> 9;
        let name_addr = symbols_base + name_off as u64;
        let remaining = fixups.size - (name_addr - fixups.header_addr);
        names.push(read_cstr(name_addr, remaining));
    }
    Ok(names)
}
    ...
    loop {
        ...
        match pointer_format {
            ...
            | DYLD_CHAINED_PTR_ARM64E_USERLAND24 => {
                return Err(Error::Unsupported(format!(
                    "arm64e pointer format {:#x} (in-process linker)",
                    pointer_format
                )));
            }
            other => {
...
fn apply_bind_stream(
    ...
    let do_bind_one =
        |symbol_name: &str, bind_type: u8, addend: i64, cursor: u64| -> Result<(), Error> {
            ...
            if std::env::var("CHIMERA_DYLD_TRACE").is_ok() {
                eprintln!(
                    "chimera: bind {} @ {:#x} -> {:#x}",
...
/// guest's `blr x8` (where `x8 = desc->thunk`) lands in this routine with
/// the descriptor pointer in `x0` per the Darwin TLV ABI. We hand back the
/// address of the thread-local slot — allocating and initialising the
/// per-thread block on the first access.
unsafe extern "C" fn chimera_tlv_get_addr(desc: *const TlvDescriptor) -> *mut u8 {
    let desc = unsafe { &*desc };
    let mut block = unsafe { libc::pthread_getspecific(desc.key) } as *mut u8;
    if block.is_null() {
        ...
        // Allocate per-thread block. We use the system allocator; threads
        // never free this — it lives until thread exit, when libsystem's
        // pthread cleanup runs (we don't register a destructor, so it
        // leaks at thread exit; with a single-threaded guest that's fine).
        ...
        unsafe {
            ...
            libc::pthread_setspecific(desc.key, block as *mut libc::c_void);
        }
    }
...
fn setup_tlv(image: &LoadedMachO, cmds: &LoadCommands) -> Result<(), Error> {
...

    {
        ...
        map.insert(key, TlvImage { template });
    }

    let thunk = chimera_tlv_get_addr
        as unsafe extern "C" fn(*const TlvDescriptor) -> *mut u8
        as usize as u64;
