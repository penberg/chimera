//! The Linux syscall policy: the driver the arch dispatcher calls once per
//! guest syscall instruction. It services the calls Chimera must own itself
//! (`exit`/`exit_group`, `execve`/`execveat`, `arch_prctl`, the `mmap` family,
//! `clone`/`fork`/`vfork`, the `rt_sig*` family, and the interfaces refused for
//! W^X or translation safety) and hands everything else to the embedder hooks.
//! The neutral `SystemCall`/`SystemCalls` surface it drives lives in
//! [`crate::syscall`]; a second host supplies its own `policy::syscall` behind
//! the same neutral `sys::policy` name.

use std::ptr;

use crate::{
    SyscallResult, SystemCall, SystemCalls,
    arch::dispatch::{CLONE_ARGS_SIZE_MAX, RSP, Thread, read_clone3_args},
    host_syscall,
    sys::mmap::copy_from_guest,
};

use super::exec::{exec_errno, prepare_exec};
...
pub fn syscall(thread: &mut Thread, call: &mut SystemCall, handler: &dyn SystemCalls) {
    ...
    match nr {
        ...
        libc::SYS_execve | libc::SYS_execveat => {
            ...
            match prepare_exec(call.number, &call.args, handler) {
                Ok(prepared) => {
                    ...
                    // next boundary, before the guest could read rax. The
                    // returned loser's drop closes its parsed image.
                    let _ = thread.process().request_exec(prepared, &thread.state);
        ...
        libc::SYS_vfork => {
            ...
            call.args = [libc::SIGCHLD as u64, 0, 0, 0, 0, 0, 0, 0];
            forked(thread, call, handler, 0);
        }
        ...
        libc::SYS_personality
            if call.args[0] as libc::c_uint != 0xffff_ffff
                && call.args[0] as libc::c_uint & libc::READ_IMPLIES_EXEC as libc::c_uint != 0 =>
        {
            call.set_result(SyscallResult::Error(libc::EPERM));
        }
        libc::SYS_arch_prctl => {
        ...
        libc::SYS_rt_sigprocmask => {
            let r =
                thread
                    .signals_mut()
                    .sigprocmask(call.args[0] as i32, call.args[1], call.args[2]);
            call.set_result(r);
        }
