...
- **Conformance**: `make conformance` builds Chimera and runs each test under it; `make conformance-native` runs the same tests directly without Chimera; `make conformance-release` runs them under an optimized build, which is not interchangeable with the debug one — the known intermittent failure reproduces only there.
