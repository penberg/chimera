//! Reading and replaying a delta: the whiteout and origin markers are
//! xattrs, and applying a change restores them, so this half of the `fs`
//! tooling is Linux-only until those interfaces are ported.
